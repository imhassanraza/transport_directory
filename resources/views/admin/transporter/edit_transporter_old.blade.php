                            ...
                            <div class="col-md-12">
                                <table id="table_lists_tbl" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                                    ...
                                    <tbody id="vehicles_list">
                                        ...
                                        <tr @if($i != 0) class="new_added_rows" @endif id="tr_{{ $i }}">
                                            <td>
                                                <select class="form-control" name="vehicles[{{ $i }}][vehicle_type]" id="vehicle_type_{{ $i }}" style=" width: 100% !important;">
                                                    <option value="">Select Vehicle Type</option>
                                                    @foreach(get_complete_table('vehicle_types', '', '', '', '') as $type)
                                                    <option value="{{ $type->id }}" @if($type->id == $vhl->vehicle_type) selected @endif>
                                                        {{ $type->vehicle_type }}
                                                    </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td>
                                                <select class="form-control select2_demo_3" name="vehicles[{{ $i }}][vehicle_city]" id="vehicle_city_{{ $i }}" style=" width: 100% !important;">
                                                    <option value="">Select City</option>
                                                    @foreach(get_complete_table('cities', '', '', 'name', 'asc') as $vcity)
                                                    <option value="{{ $vcity->id }}" @if($vcity->id == $vhl->vehicle_city) selected @endif>
                                                        {{ $vcity->name }}
                                                    </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td>
                                                <input type="text" class="form-control" name="vehicles[{{ $i }}][vehicle_number]" id="vehicle_number_{{ $i }}" value="{{ $vhl->vehicle_number }}" style=" width: 100% !important;">
                                            </td>
                                            <td>
                                                @if($i == 0)
                                                <button class="btn btn-primary btn-circle" onclick="add_more_rows();" type="button"><i class="fa fa-plus"></i></button>
                                                @else
                                                <button class="btn btn-danger btn-circle remove_added_rows" type="button"><i class="fa fa-minus-circle"></i></button>
                                                @endif
